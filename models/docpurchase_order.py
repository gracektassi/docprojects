    ...
    def _check_products(self):
        for order in self:
            if order.state == 'waiting_for_hod' and not order.order_line:
                raise ValidationError("You must add products to the order before submitting.")
        ...
        for order in self:
            ...
            if order.state == 'draft':
                ...
                self.env['mail.activity'].create({
                    ...
                    'res_id': order.id,
                    'user_id': order.department_id.manager_id.user_id.id if order.department_id.manager_id else False,
